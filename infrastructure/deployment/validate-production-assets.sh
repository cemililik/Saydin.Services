...
caddy_validation_image="saydin-caddy-validation:local"
...
cleanup() {
  if [[ -n "${validation_tmp:-}" && -d "$validation_tmp" ]]; then
    case "$validation_tmp" in
      ...
      *) echo "production_assets_validation_failed:unsafe_cleanup" >&2; return 64 ;;
    esac
  fi
}
trap cleanup EXIT
...
docker run --rm --read-only --cap-drop ALL --security-opt no-new-privileges \
  ...
  "$prometheus_image" test rules /etc/prometheus/tests/rules.test.yml
docker run --rm --read-only --cap-drop ALL --security-opt no-new-privileges \
