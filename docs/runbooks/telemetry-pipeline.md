...
1. Confirm `otel-collector`, `tempo`, and `loki` health/restart state on the internal
...
`SaydinWatchdog` is intentionally always firing. The external dead-man's-switch receiver
must see it every minute and page on absence; never silence or inhibit this route during a
normal incident. If the external heartbeat is missing, verify Prometheus `/-/ready`, rule
health, Alertmanager `/-/ready`, and the private watchdog receiver route from the internal
control plane. Resolve only after the external service confirms consecutive heartbeats.
